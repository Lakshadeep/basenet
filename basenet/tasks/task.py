...
class Task(Environment):
    ...
    def __init__(
        ...
        if self.cfg.task.subtask == 'single_obj':
            ...
            action_space = spaces.Box(low=np.array([0, 0, 0, 0, 0]),
    ...
    def get_graph_data(self, table_length=2.0, table_width=0.8, robot_r_max=3.0, robot_r_min=1.8, safety_dist=0.1):
        np.random.seed()
        ...
        r_theta = np.random.uniform(-np.pi, np.pi)
        robot_x = robot_r * np.cos(r_theta)
        robot_y = robot_r * np.sin(r_theta)

        
        # This should ensure that object stays in the field of view when we reset
        robot_theta = self._wrap_angle(r_theta + np.random.uniform(-np.pi/9, np.pi/9) + np.pi)
        self._move_robot_base_to_pose(robot_x, robot_y, robot_theta)
        ...
        object_poses = [self.cracker_box_prim.get_world_pose(), 
                        self.bleach_cleanser_prim.get_world_pose(),
                        self.mustard_bottle_prim.get_world_pose(),
                        self.mug_prim.get_world_pose(),
                        self.bowl_prim.get_world_pose()]
        
        # disabling this increases speed by 3 times
        self.world.step(render=self.cfg.task.mdp.render)
        return self.obj_list, robot_base_pose, table_pose, object_poses 
    ...
    def get_current_graph(self):
        ...
        object_poses = [self.cracker_box_prim.get_world_pose(), 
                        ...
                        self.mug_prim.get_world_pose(),
                        self.bowl_prim.get_world_pose()]
        G = self.get_state_graph(self.obj_list, robot_base_pose, table_pose, object_poses)
        return G
    ...
    # used for embeddings
    # GNN tasks use one in the networks
    # Grpah2vec only works for graphs with same number of nodes
    def get_state_graph(self, state):

        G = nx.Graph()
        
        # add robot node
        G.add_node(int(state[3]), x=state[0:4])

        # add object nodes
        for idx in range(0, 5):
            i = (idx * 4) + 4

            G.add_node(int(state[i+3]), x=state[i:i+4])
            
            # overriden for now
            # if obj_status:
            #     G.add_edge(0, id)
        
        # # only connect cracker box and robot
        G.add_edge(1, 0)
